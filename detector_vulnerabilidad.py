...
def classify_device(rdns: str, netbios: str, banners: Dict[int, str], vendor: str) -> str:
    ...
    if any(p in banners for p in [80, 443, 8080]):
        return "Servidor Web"
...
def score_risk(reachable: bool, banners: Dict[int, str], crypto: bool) -> Tuple[int, List[str], List[str]]:
    score = 0
    findings = []
    recs = []
    for port in banners:
...
def scan_ip(ip: str, ports: List[int], timeout: float, retries: int, skip_ping: bool = False, try_arp: bool = True, crypto_scan: bool = False, crypto_ports: Optional[List[int]] = None) -> Dict:
    ...
    result.status = "reachable" if result.reachable else "no_response"
    ...
    if not result.reachable and not result.rdns and not result.netbios and not result.mac:
        result.status = "unreachable"
    elif result.reachable and not result.banners:
        result.status = "no_banner"
    elif result.reachable and result.banners:
        result.status = "ok"
    ...
    result.risk_score, result.findings, result.recommendations = score_risk(result.reachable, result.banners, result.crypto_suspicion)
...
def save_csv(results: List[Dict], out_prefix: str) -> str:
    ...
    with open(path, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["ip", "status", "reachable", "reverse_dns", "netbios", "mac", "vendor", "device_type", "risk_score", "risk_level", "crypto_suspicion", "findings", "recommendations", "banners"])
        for r in results:
            banners = ";".join([f"{p}={str(banner)[:200]}" for p, banner in r.get("banners", {}).items()])
            writer.writerow([r.get("ip", ""), r.get("status", "unknown"), r.get("reachable", False), r.get("rdns", ""), r.get("netbios", ""), r.get("mac", ""), r.get("vendor", ""), r.get("device_type", ""), r.get("risk_score", 0), r.get("risk_level", "low"), r.get("crypto_suspicion", False), ";".join(r.get("findings", [])), ";".join(r.get("recommendations", [])), banners])
    return path
...
def save_summary(results: List[Dict], out_prefix: str) -> str:
    ...
    status_count = Counter(r.get("status", "unknown") for r in results)
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(f"No response: {status_count.get('no_response', 0)}\\n")
        f.write(f"No banner: {status_count.get('no_banner', 0)}\\n")
        f.write(f"Ok: {status_count.get('ok', 0)}\\n\\n")
...
def main() -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
        ...
        for ip in ips:
            ...
            futures[executor.submit(scan_ip, ip, ports, args.timeout, args.retries, args.skip_ping, not args.no_arp, args.crypto_scan, crypto_ports)] = ip
        ...
        for fut in iterator:
            ...
            try:
                ...
                results.append({"ip": ip, "status": "error", "reachable": False, "rdns": "", "netbios": "", "mac": "", "vendor": "", "device_type": "", "risk_score": 0, "risk_level": "low", "findings": [str(e)], "recommendations": [], "banners": {}, "crypto_suspicion": False, "crypto_ports": [], "crypto_details": []})
            done += 1
